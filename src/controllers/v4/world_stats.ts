...
import { FastifyReplyTypeBox, FastifyRequestTypeBox } from "#controllers/v4/types.js";
import {
...
export class WorldStatsController {
	...
	getWorldListStats = async (
		...
		return await WorldsStatsService.getWorldListStats(
			this.worldStatsCollection,
			{ project },
		);
	};
	...
	getWorldStats = async (
		...
		const worldStats = await WorldsStatsService.getWorldStats(
			this.worldStatsCollection,
			{ world_uuid, project },
		);
		if (!worldStats)
			throw new ApiError(`World stats for world ${world_uuid} not found`, 404);
